...
def process_forecast(date: str = "2026-09-10", lead_time: int = 24, model_source: str = "GFS"):
    ...
    return {"date": date, "lead_time": lead_time, "model_source": model_source, "regime": regime, "districts": corrected_districts}
...
def get_probability_map(date: str, lead_time: int = 24):
    ...
    for d in forecast["districts"]:
        corrected = bias_corr.predict(d["raw"], regime["features"], regime["type"], lead_time)
        probs = prob_est.predict(corrected, regime["features"], lead_time)
        districts.append({"district_id": d["district_id"], "name": d["name"], "state": d["state"], "lat": d["lat"], "lon": d["lon"], "corrected": corrected, **probs})
...
def get_verification_report(date: str, lead_time: int = 24):
    ...
    for regime in ["active_monsoon", "break_monsoon", "depression", "orographic", "coastal", "western_disturbance"]:
        ...
        by_regime[regime] = {"raw": compute_verification_metrics(obs_r, raw_r, 64.5), "corrected": compute_verification_metrics(obs_r, corr_r, 64.5)}
    by_lead_time = []
    for lt in [24, 48, 72, 96, 120]:
        obs_lt = rng.exponential(30, 100)
        noise = 4 + (lt - 24) / 20.0
        raw_lt = obs_lt * rng.uniform(0.8, 1.5, 100) + rng.normal(0, noise, 100)
        corr_lt = obs_lt * rng.uniform(0.9, 1.15, 100) + rng.normal(0, noise * 0.6, 100)
        raw_m = compute_verification_metrics(obs_lt, raw_lt, 64.5)
        corr_m = compute_verification_metrics(obs_lt, corr_lt, 64.5)
        by_lead_time.append({"lead": f"T+{lt}", "raw_rmse": raw_m["rmse"], "corrected_rmse": corr_m["rmse"], "raw_ets": raw_m["ets"], "corrected_ets": corr_m["ets"]})
    return {"date": date, "lead_time": lead_time, "overall": {"raw": raw_metrics, "corrected": corrected_metrics}, "by_regime": by_regime, "by_lead_time": by_lead_time}
